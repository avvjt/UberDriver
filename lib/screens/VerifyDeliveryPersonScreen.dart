...
class VerifyDeliveryPersonScreenState extends State<VerifyDeliveryPersonScreen> {
  ...
  addDocument(int? docId, int? isExpire, {int? updateId, DateTime? dateTime}) async {
    ...
    if (image != null) {
      multiPartRequest.files.add(await MultipartFile.fromPath("driver_document", image!.path));
    }
                            ...
                            items: documentList.map((e) {
                              return DropdownMenuItem(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      children: [Text(e.name.validate(), style: primaryTextStyle()), SizedBox(width: 4), Text('${e.isRequired == 1 ? '*' : ''}', style: boldTextStyle(color: Colors.red))],
                                    ),
                                  ],
                                ),
                              );
